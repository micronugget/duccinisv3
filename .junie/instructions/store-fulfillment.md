...
> Use when writing or modifying code in `web/modules/custom/store_fulfillment/` — Commerce checkout panes,
> shipping plugins, delivery radius validation, AJAX multi-pane patterns, event subscribers, and payment radios.
...
Panes shown/hidden via AJAX must always return `TRUE` from `isVisible()`. Returning `FALSE` causes Commerce's `CheckoutFlowWithPanesBase::buildForm()` to call `Element::getVisibleChildren()` which removes the wrapper `<div>` from the DOM entirely, breaking AJAX targeting.
...
**Never** add to `#process` on a pre-existing radios element. It creates a brand-new array overwriting Drupal's defaults, dropping `Radios::processRadios()` and producing zero child radio elements.
