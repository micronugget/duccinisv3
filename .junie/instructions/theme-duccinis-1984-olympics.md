...
> Use when writing or modifying files in `web/themes/custom/duccinis_1984_olympics/` —
> Twig templates, SCSS, SDC components, JS behaviors, preprocess functions, and library definitions.
> Covers the build pipeline, the `#after_build` ↔ Twig variable chain for saved-card display,
> and the CSS-only `:checked` selection state pattern.
...
`element_id` is the `id` attribute of the hidden `<input>` — used in the
Twig template as `for="{{ element_id }}"` on the `<label>`.
...
stops working with no JS error to debug.

---

## CSS-Only `:checked` Selection State — Rules

The saved-card selected state uses **no JavaScript**. These CSS patterns make it work:

```scss
// Correct: radio immediately precedes the label.
.saved-card__radio:checked + .saved-card {
...
## Key Architecture Constraints (Non-Negotiable)

Breaking these silently breaks checkout UI:
...
5. **SDC CSS compiles to same directory** — `components/saved-card/saved-card.scss` → `components/saved-card/saved-card.css`.
...
fires, so Drupal's AJAX handler never runs.
...
The `core/drupalSettings` dependency in `libraries.yml` is **required** — removing
it changes JS asset load order and may cause the behavior to attach before Drupal
AJAX bindings are registered.
...
Commerce ships `commerce_checkout.form.css` that applies `float: left; width: 65%` and related rules. Our theme also applies `.commerce-checkout-flow .layout-region { padding: 2rem }`. A custom CSS Grid rule stacks on top of both — it does not replace them, creating compounding whitespace.

The right approach is to override the Twig template to emit Bootstrap markup:
...
**Rules:**
1. Use `col-lg-*` (992px breakpoint) — `col-md-*` at 720px inner width is too narrow for a sidebar.
2. Use `g-3` or `g-4` for gutters — never add compensating `padding`/`margin` to SCSS.
3. Retain `layout-region` and `layout-region-checkout-*` classes so Commerce JS continues to work.
4. Keep `layout-checkout-form` on the outer wrapper.
...
by declaring library entries in `libraries.yml` with the **same machine names**
...
- `$olympics-blue: #0077C0`
- `$olympics-magenta: #D62976`
